...
363. 弱网情况下怎么做优化。
364. 各个模块是怎么做隔离的，具体的实现。
365. LeakCanary的原理，解决过哪些内存泄漏的东西。
364.mvp和mvvm的区别，mvvm中view是怎么观察m的，mvvm需要感知声明周期吗？mvp需要感知声明周期吗？
365.进程间通讯的方式，AIDL怎么写，如果B进程主动调用A进程的东西怎么写？
366.影响中最深的一次启动优化。
367. k个一组翻转列表。

368. 说下stage模型，开发模式跟Android有什么不同。
369. 鸿蒙S标。
370. ExtentsionAbility和UIAbility。
371. IM产品需要关注的性能指标有哪些，怎么统计的。
372. 有没有跟产品和设计合作，怎么合作的。
373. 翻转列表。

374. 点击一个APP的启动流程。
375. glide的原理
376. lwp的原理
377. IM的整体架构，读扩散和写扩散的区别。

378. java和C++怎么相互持有。
379. ANR的种类，怎么捕获ANR，怎么判断ANR。
380. 怎么判断一个ip地址是否是合法的ip地址。
381. 怎么判断内存泄漏，以及内存泄漏怎么dump，怎么保证不卡。
382. java层和c++层分别怎么捕获crash。
383. java层怎么调用c++层，已经c++层怎么调用java层。
